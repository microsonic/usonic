...
RUN --mount=type=cache,target=/var/cache/apt --mount=type=cache,target=/var/lib/apt \
apt update && apt install -qy strace vim gdb procps redis-server syslog-ng tcpdump socat wget rlwrap

RUN wget --no-use-server-timestamps -O /usr/bin/dsserve 'https://sonicstorage.blob.core.windows.net/packages/20190307/dsserve?sv=2015-04-05&sr=b&sig=lk7BH3DtW%2F5ehc0Rkqfga%2BUCABI0UzQmDamBsZH9K6w%3D&se=2038-05-06T22%3A34%3A45Z&sp=r' && chmod +x /usr/bin/dsserve
