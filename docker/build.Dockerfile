...
RUN --mount=type=bind,target=/root,rw cd /root && quilt push -a

RUN --mount=type=bind,target=/root,rw make -C /root/make/swss-common
RUN --mount=type=bind,target=/root,rw make -C /root/make/sairedis
RUN --mount=type=bind,target=/root,rw make -C /root/make/swss
